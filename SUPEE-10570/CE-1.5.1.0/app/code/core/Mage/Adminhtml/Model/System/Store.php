        ...
        foreach ($this->_websiteCollection as $website) {
            ...
            foreach ($this->_groupCollection as $group) {
                ...
                foreach ($this->_storeCollection as $store) {
                    ...
                    $values[] = array(
                        'label' => str_repeat($nonEscapableNbspChar, 4) . Mage::helper('core')->escapeHtml($store->getName()),
                        'value' => $store->getId()
                    );
                }
                if ($groupShow) {
                    $options[] = array(
                        'label' => str_repeat($nonEscapableNbspChar, 4) . Mage::helper('core')->escapeHtml($group->getName()),
                        'value' => $values
                    );
                }
            }
